            ...
            if(typeArray.includes(ext)){
                ...
                s3.upload(params, function(err, data) {
                    if (err) {
                        ...
                        s3logger.error(`${req.user.accout_type} ${req.user.email} try to upload ${fname} got error.`)
                    }else{
                        if(limitCrossed){
                            const delparam = {
                                Bucket: process.env.BUCKET_NAME,
                            ...
                            s3.deleteObject(delparam,(err,data)=>{
                                 if (err){
                                     s3logger.error(`${req.user.accout_type} ${req.user.email} error while deleting ${fname} due to file size limit exceeds.`)
                                 }else{
                                     s3logger.info(`${req.user.accout_type} ${req.user.email} successfully deleted ${fname} due to file size limit exceeds.`)
                                 }
                            })
                        }else{
                            res.json({status:200,photo:data.Location})

                            s3logger.info(`${req.user.accout_type} ${req.user.email} successfully updated profile photo to ${fname} `)
                            ...
                            Model.findOneAndUpdate({_id:req.user._id},
                                ...
                                (err,user)=>{
                                    if(err){
                                        winslogger.error(`${req.user.accout_type} ${req.user.email} error while updating photo url`)
                                    }else if(user){
                                        if(req.user.photo !== null){
                                            ...
                                            s3.deleteObject(delparam2,(err,data)=>{
                                                if (err){
                                                    s3logger.error(`${req.user.accout_type} ${req.user.email} error while deleting ${fname} after updating with new one`)
                                                }else{
                                                    s3logger.info(`${req.user.accout_type} ${req.user.email} successfully deleted ${fname} after updating with new one`)
                                                }
                                            })
                                        }
                                    }
